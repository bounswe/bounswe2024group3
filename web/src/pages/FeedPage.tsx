...
export const FeedPage = () => {
  ...
  const fetchPosts = async (page: number) => {
    try {
      ...
      const endpoint =
        selectedTab === "Following" ? "get-following-posts/" : "get-posts/";
      const response = await req(
        `${endpoint}?page=${page}&page_size=10`,
        "get",
        {}
      );
      const newPosts: PostDetails[] = response.data.posts;
  ...
  return username ? (
    <div className="flex justify-center max-w-screen-xl mx-auto w-full gap-6">
      ...
      <div className="w-64 hidden lg:block">
        ...
        <Link
          to="/map"
          className="btn btn-primary w-full mb-4 flex items-center"
        >
          <svg
            version="1.1"
